...
export namespace IScript {
  export const SCENARIO: IScript[] = [
    {
      name: "Administrator",
      message: "The Chat Application has been started.",
    },
    {
      name: "Jeongho Nam",
      message: "Hello, my name is Jeongho Nam, author of the TGrid.",
    },
    { name: "Sam", message: "I'm Sam, nice to meet you." },
    {
      name: "Administrator",
      message: "Welcome two participants. Nice to meet you too.",
    },
    ...
    {
      name: "Jeongho Nam",
      message: "I'm going to sleep. See you tomorrow.",
    },
  ...
  export const PEOPLE: string[] = [
    ...new Set(SCENARIO.map((script) => script.name)),
  ];
  ...
  export function validate(scriptList: IScript[]): void {
    ...
    for (const script of scriptList) {
      const index: number = SCENARIO.findIndex((s) => {
        return s.name === script.name && s.message === script.message;
      });
      if (index === -1) throw new DomainError("Different script exists.");
    }
  }
}
