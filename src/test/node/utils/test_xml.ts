...
class Invoke extends Vector<Parameter> {
    ...
    public toXML(): XML {
        ...
        for (const param of this) ret.push(param.toXML());
...
class Parameter {
    ...
    public toXML(): XML {
        ...
        if (this.value !== null)
            if (this.value instanceof XML) ret.push(this.value as XML);
            else ret.setValue(this.value as string);
...
function validate_equality<T>(x: T, y: T): void {
    if (x !== y) throw new DomainError("Error on XML Parser.");
}
...
function write(): Pair<Invoke, Vector<Member>> {
    ...
    for (const elem of members) memberList.push(elem.toXML());
...
function read(pair: Pair<Invoke, Vector<Member>>): void {
    ...
    //----
    xmlList = xml.get("parameter").at(2).get("memberList").at(0).get("member");
