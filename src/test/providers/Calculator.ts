...
export class Calculator extends Simple implements ICalculator {
...
export class Scientific implements IScientific {
    ...
    public log(x: number, y: number): number {
        if (x < 0 || y < 0) throw new Error("Negative value on log.");
...
export class Statistics implements IStatistics {
    public mean(...elems: number[]): number {
        let ret: number = 0;
        for (const val of elems) ret += val;
    ...
    public stdev(...elems: number[]): number {
        ...
        for (const val of elems) ret += Math.pow(val - mean, 2);
