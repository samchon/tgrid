    ...
    public async open(
        ...
        if (this.state_ === WebServer.State.OPEN)
            ...
            throw new RuntimeError(
                "Error on WebServer.open(): it's on closing.",
            );
        // DO OPEN
        else if (
            this.server_ === null ||
            ...
            this.server_ =
                this.options_ !== null
                    ? (await NodeModule.https.get()).createServer(
                          this.options_!,
                      )
                    ...
                    (webSocket) => {
                        webSocket.once(
                            ...
                            async (data: WebSocket.Data) => {
                                ...
                                try {
                                    const wrapper: IHeaderWrapper<Header> =
                                        JSON.parse(data as string);
                                    const acceptor: WebAcceptor<
                                        Header,
                                        Provider
                                    > = WebAcceptor.create(
                                        request,
